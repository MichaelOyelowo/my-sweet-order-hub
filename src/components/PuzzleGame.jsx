...
function SlidingPuzzle({ onWin }) {
  ...
  // Lost when timer hits 0
  useEffect(() => {
    if (timeLeft === 0) onWin(false)
  }, [timeLeft])
      ...
      <div className="puzzle-grid">
        {tiles.map((val, idx) => {
          ...
          return (
            ...
            >
              {!isEmpty && <>
                <span className="pt-num">{val + 1}</span>
                <span className="pt-lbl">{TILE_LABELS[val]}</span>
              </>}
            </button>
          )
        })}
...
function MemoryMatch({ onWin }) {
  ...
  useEffect(() => {
    ...
    if (cards[a].id === cards[b].id) {
      const newMatched = [...matched, cards[a].id]
      setMatched(newMatched)
      setFlipped([])
      setLocked(false)
      if (newMatched.length === 8) {
        clearInterval(timerRef.current)
        setTimeout(() => onWin(true), 400)
      }
    } else {
...
// ════════════════════════════════════════════════════════════
// GAME 3 — CATCH THE SNACKS
// ════════════════════════════════════════════════════════════
const BASKET_W   = 80
const GAME_W     = 320
const ITEM_SIZE  = 36
const FALL_SPEED = 3
const SPAWN_RATE = 1400 // ms between spawns
const WIN_SCORE  = 15
const MAX_MISS   = 5
...
// REWARD SCREEN
// ════════════════════════════════════════════════════════════
function RewardScreen({ moves, onClose }) {
  ...
  if (chosen) return (
    <div className="puzzle-screen puzzle-claimed">
      ...
      {/* The WhatsApp button is now safely INSIDE the main div */}
      <a 
    ...
    </div> /* <-- Notice this final closing tag that wraps EVERYTHING! */
  )
...
const GAME_CONFIGS = [
  {
    ...
    time: '3 minutes',
    tip: 'Use arrow keys or click tiles',
    color: '#c0392b',
  },
  {
    id: 'memory',
    name: 'Memory Match',
    emoji: '🃏',
    tagline: 'Find all matching food pairs',
    time: '2 minutes',
    tip: 'Remember where each food hides',
    color: '#8e44ad',
  },
  {
    id: 'catch',
    name: 'Catch the Snacks',
    emoji: '🍿',
    tagline: 'Catch 15 snacks in your basket',
    time: '60 seconds',
    tip: 'Move mouse or arrow keys',
    color: '#e67e22',
  },
]
...
export default function PuzzleGame() {
  const [phase, setPhase]     = useState('hidden')    // hidden → intro → playing → won → lost → reward
  const [gameId, setGameId]   = useState(null)
  const [result, setResult]   = useState(null)        // { won, moves }

  // Randomly assign game on show
  useEffect(() => {
    if (sessionStorage.getItem(STORAGE_KEY)) return
    const t = setTimeout(() => {
      const picked = GAME_CONFIGS[Math.floor(Math.random() * GAME_CONFIGS.length)]
      setGameId(picked.id)
      setPhase('intro')
  ...
  return (
    <div className="puzzle-overlay" role="dialog" aria-modal="true" aria-label="SweetHUB Game Challenge">
      <div className="puzzle-modal">

        {/* ── INTRO ── */}
        {phase === 'intro' && config && (
          <div className="puzzle-screen puzzle-intro">
            <button className="puzzle-x" onClick={close} aria-label="Close">×</button>

            <div className="puzzle-intro-badge" style={{ background: `linear-gradient(135deg, ${config.color}, ${config.color}cc)` }}>
              🎮 SweetHUB Game Challenge
            </div>

            <div className="game-intro-icon" style={{ background: `${config.color}18`, border: `2px solid ${config.color}33` }}>
              <span>{config.emoji}</span>
            </div>

            <h2 className="puzzle-intro-title">
              {config.name}<br />
              <span style={{ color: config.color }}>Win free food!</span>
            </h2>

            <p className="puzzle-intro-desc">
              {config.tagline}. You have <strong>{config.time}</strong> to win.
              Beat the challenge and choose your own reward! 🎁
            </p>

            <div className="puzzle-intro-rules">
              <div className="pir"><span>⏱️</span><span>{config.time} on the clock</span></div>
              <div className="pir"><span>💡</span><span>{config.tip}</span></div>
              <div className="pir"><span>🏆</span><span>Win = choose your own reward</span></div>
              <div className="pir"><span>🎲</span><span>A different game awaits each visit!</span></div>
            </div>
            ...
            <button className="puzzle-skip" onClick={close}>No thanks, maybe later</button>
          </div>
        )}

        {/* ── PLAYING ── */}
        {phase === 'playing' && (
          <div className="puzzle-screen" style={{ padding: '20px 16px' }}>
            <div className="game-playing-header">
              <span className="game-playing-name">{config?.emoji} {config?.name}</span>
              <button className="puzzle-give-up" onClick={close}>Give up 😅</button>
            </div>

            {gameId === 'puzzle' && <SlidingPuzzle onWin={handleGameEnd} />}
            {gameId === 'memory' && <MemoryMatch   onWin={handleGameEnd} />}
            {gameId === 'catch'  && <CatchSnacks   onWin={handleGameEnd} />}
          </div>
        )}

        {/* ── WON ── */}
        {phase === 'won' && (
          <RewardScreen onClose={close} />
        )}
        ...
        {phase === 'lost' && (
          <div className="puzzle-screen puzzle-result lost">
            ...
            <p className="puzzle-result-sub">
              You almost had it! 😅 Don't worry — a new game challenge
              will appear on your next visit. Keep playing!
            </p>
            <div className="puzzle-lost-btns">
              <button className="puzzle-retry-btn" onClick={retry}>Try Again 🔄</button>
              <button className="puzzle-skip" onClick={close}>Maybe next time</button>
            </div>
          </div>
        )}
